        ...
        for table in tables:
            ...
            for row in table:
                ...
                for target, keywords in TARGET_MAP:
                    ...
                    if hit:
                        # --- v72.0 關鍵修正：只掃描項目名稱之後的格子 ---
                        # 這樣 "Cr 6+" 這一格就不會被掃描，避免 "6" 被抓取
                        cells_to_scan = clean_row[hit_cell_index+1:]
                        
                        val = scan_row_for_value(cells_to_scan)
                        if val:
...
# --- 4. SGS/CTI/Generic 解析模組 (保留 v53/v49 邏輯) ---
...
if __name__ == "__main__":
    st.set_page_config(page_title="SGS/CTI/Intertek Tool v72.0", layout="wide")
    st.title("📄 萬用型檢測報告聚合工具 (v72.0 Intertek Cr6+ 修復版)")
    st.info("💡 v72.0：修復 Intertek Cr6+ 誤抓問題（自動避開項目名稱中的 'Cr 6+' 數字）。SGS 與 CTI 邏輯完全保留未動。")
