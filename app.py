...
def extract_clean_report_date(text):
    """
    v47.0: 行級過濾 (Line-Level Filtering)
    只有當該行包含 "Date" 且 不包含 "Approved/Received" 時，才提取日期。
    """
    lines = text.split('\n')
    valid_dates = []
    ...
    # 必須包含的關鍵字
    allow_kw = ["date", "issue", "report", "日期"]
    
    # 絕對禁止的關鍵字 (只要這行出現這些字，這行的日期就作廢)
    block_kw = [
        "approve", "approved", "approval", "approver", # 承認/核准
        "check", "checked", "review", "reviewed",      # 審核/檢查
        "receive", "received", "receipt",              # 收件 (通常比較早，排除以防萬一)
        "承认", "核准", "檢驗", "收件", "有效", "expiry", "validity"
    ]
    ...
    for line in lines:
        ...
        # 2. 白名單檢查 (Allow)
        if not any(good in line_lower for good in allow_kw):
            continue
    ...
    # 經過清洗後，剩下的應該只有 Test Period 和 Report Date
    # 取 max 應該就是 Report Date
    if valid_dates:
        ...
        try:
            with pdfplumber.open(file) as pdf:
                ...
                for p_idx in range(len(pdf.pages)):
                    ...
                    # v47.0: 前5頁嚴格提取日期
                    if p_idx < 5:
                        d = extract_clean_report_date(page_txt)
...
# --- 介面 ---
st.set_page_config(page_title="SGS 報告聚合工具 v47.0", layout="wide")
st.title("📄 萬用型檢測報告聚合工具 (v47.0)")
st.info("💡 v47.0：精準日期過濾，嚴格剔除 Approved/承認日期，確保只抓取 Report Date。")
...
if uploaded_files:
    ...
    try:
        ...
        st.download_button("📥 下載 Excel", data=output.getvalue(), file_name="SGS_Summary_v47.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
